...
int main(void){
	...
	/* SHADOW */
	#ifdef LOGO_SHADOW
	shadowSpr = sprMakeDefault(get_current_sprite());
	shadowSpr.pos.x = (LOGO_XPOS - 40) + (logoZoomEffect.zoom / 2);
	shadowSpr.pos.y = (LOGO_YPOS + 15) - (logoZoomEffect.zoom / 4);
	shadowSpr.size = vec2u16Make(304, 96);
	shadowSpr.zoom.x = 15;
	shadowSpr.zoom.y = (255 - 1 * 2) + 90;
	write_sprite_data(shadowSpr.pos.x, shadowSpr.pos.y, shadowSpr.zoom.x, shadowSpr.zoom.y, tileSizeClipping(shadowSpr.size.y),tileSizeNB(shadowSpr.size.x), (const PTILEMAP)&shadow);
	#endif
	...
	while(1){
		...
		shadowSpr.pos.y = (LOGO_YPOS + 15) - (logoZoomEffect.zoom / 4);
		shadowSpr.zoom.x = 15;
		shadowSpr.zoom.y = (255 - logoZoomEffect.zoom * 2) + 90;
		shadowSpr.size = vec2u16Make(96, 304);
		change_sprite_pos(shadowSpr.idSpr, shadowSpr.pos.x, shadowSpr.pos.y, shadowSpr.clipping);
		change_sprite_zoom(shadowSpr.idSpr, shadowSpr.zoom.x, shadowSpr.zoom.y, tileSizeNB(304));
		#endif
