...
void sprLink(spr *prevSpr, spr *nextSpr){
	prevSpr->next = nextSpr;
	nextSpr->prev = prevSpr;
}
#endif
...
spr *sprListAdd(spr *spriteBefore, spr *spriteAfter){
	if (spriteBefore != NULL && spriteAfter != NULL){
		spriteBefore->next = spriteAfter;
		spriteAfter->prev = spriteBefore;
	}
	if (spriteAfter != NULL){
		return spriteAfter;
	}
...
WORD sprListRemove(spr *sprite){
	if (sprite != NULL){
		if (sprite->prev == NULL && sprite->next == NULL){// if sprite is alone
			free(sprite);
			return 0;
		}
		if (sprite->prev == NULL && sprite->next != NULL){ //If sprite is the first and not alone
			sprite->next->prev = NULL;
			free(sprite);
			return 0;
		}
		if (sprite->next == NULL && sprite->prev != NULL){ // if sprite is the the last and not alone
			sprite->prev->next = NULL;
			free(sprite);
			return 0;
		}
		if (sprite->next != NULL && sprite->prev != NULL){
